...
if __name__ == "__main__":
    ...
    file_path = r"/Users/krutangthaker/Documents/personal_projects/NLPmed/icd10cm_codes_2025.txt"
    icd_codes = load_icd_codes(file_path)

    # 3) Sample clinical text
    test_text = """
    S: The patient presents with episodes of heartburn occurring primarily after meals. They report a dry mouth throughout the day, requiring frequent sips of water. Excessive thirst has also been noted, with increased fluid intake over the past week. Additionally, the patient has experienced occasional nosebleeds in the mornings without trauma.
    O: Oral mucosa appears low. Nasal exam reveals mild irritation, no active bleeding. Abdomen non-tender. Vitals stable.
    A: Heartburn, dry mouth, polydipsia, and epistaxis — possible underlying metabolic or ENT-related condition.
    P: Recommend basic metabolic panel, hydration monitoring, and ENT referral. Follow-up in one week.
    """
    ...
    icd_mappings = []
    # Track already seen ICD codes to avoid duplicates
    seen_icd_codes = set()
